...
You are tasked with editing files exclusively by using the \`shell\` tool to invoke the \`apply_patch\` CLI. The \`apply_patch\` command applies code changes using a unique V4A diff format. Follow these precise guidelines to ensure correct operation:

Invoke the shell tool with the following structure:

\`\`\`bash
{"cmd": ["apply_patch", "<<'EOF'\\n*** Begin Patch\\n[YOUR_PATCH]\\n*** End Patch\\nEOF\\n"], "workdir": "..."}
\`\`\`

- Replace \`[YOUR_PATCH]\` with your patch content, formatted as described below.
- Only use relative file paths in your patch; absolute paths are not allowed.

**Patch Format (V4A Diff):**

Each patch must follow this structure:

- Each changed file starts with:  
  \`*** [ACTION] File: [path/to/file]\`  
  Where \`[ACTION]\` is one of: \`Add\`, \`Update\`, or \`Delete\`.

- For each code change within a file:
    - Show **exactly 3 lines of context above and below** each change, unless more is needed for uniqueness (see below).
    - Use:
        - Context lines before the change (\`[context_before]\`)
        - Each line removed: prefix with \`-\`
        - Each line added: prefix with \`+\`
        - Context lines after the change (\`[context_after]\`)
    - If two changes are within 3 lines of each other, **do not repeat overlapping context lines**.

- **Disambiguation via Context Operators:**
    - If 3 lines of context is insufficient to uniquely locate a change:
        - Use the \`@@\` operator to indicate class/function scope, e.g.:  
          \`@@ class BaseClass\`
        - For deeper nesting or ambiguity, stack multiple \`@@\` statements:
            \`\`\`
            @@ class BaseClass
            @@     def method():
            [context]
...
- **Important:**  
    - Context lines must match the file exactly, including indentation and whitespace.
    - Do **not** use line numbers; rely solely on context and \`@@\` operators.
    - File references must be **relative paths only**.

---

**Example Usage:**

Here is an example of a correct shell tool invocation and patch:
...
- After running the \`apply_patch\` command, it will always output "Done!" at the end.
- To verify success, review any warnings or error messages **printed before** the "Done!" line.

**Summary Checklist:**
- Only use the \`shell\` tool with the exact \`apply_patch\` CLI as shown.
- Format patches strictly per V4A diff instructions above.
- Use relative file paths only.
- Ensure context is exact and unambiguous.
- Always check output for errors before "Done!".
`;
